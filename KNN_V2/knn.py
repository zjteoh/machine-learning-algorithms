...
class KNN:
    def __init__(self, k, distance_function):
        ...
        """
        self.k = k
        self.distance_function = distance_functiond
        self.features = None
        self.labels = None
    ...
    def predict(self, features):
        ...
        :return: List[int]
        """
        return [np.argmax(np.bincount(self.get_k_neighbors(x))) for x in features]
    ...
    def get_k_neighbors(self, point):
        ...
        :return:  List[int]
        """
        combined = [(self.distance_function(point, x), y) for x,y in zip(self.features, self.labels)]
        combined.sort()
        return [y for _,y in combined[:self.k]]
